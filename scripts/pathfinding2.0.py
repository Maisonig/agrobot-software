...
class Grid:
    ...
    def get_neighbours(self, node):
        neighbours = []
        for x, y, p_w in self.FIVE_CELL_NEIGHBOURS:
...
class AstarFinder:
    ...
    def check_collision(self, grid, node):
        col = grid.grid[node[1] - self.robotRad:node[1] + self.robotRad, node[0] - self.robotRad:node[0] + self.robotRad]
        try:
            if col.max() == 255:
                return False
            else:
    ...
    def get_path(self, grid, start_pos, goal_pos):
        ...
        while True:
            ...
            for neighbour in neighbours:
                ...
                if grid_weight == self.NON_WALKABLE_WEIGHT or grid.closed[y, x] or not self.check_collision(grid,
                                                                                                            neighbour):
                    pass
                else:
...
while True:
    ...
    if path is not None:
        for point in path:
            cv2.circle(drawA, [point[0], point[1]], 1, [200], -1)
